...
/*****************************************************************************
	...
	delta as the brightness grows or fades.
*****************************************************************************/
...
// Make the current one white
	savePattGrn = LEDpattern[(currentChannel-LOW_CHANNEL)*3];
	savePattRed = LEDpattern[(currentChannel-LOW_CHANNEL)*3+1];
	savePattBlu = LEDpattern[(currentChannel-LOW_CHANNEL)*3+2];
	LEDpattern[(currentChannel-LOW_CHANNEL)*3] = 127;
	LEDpattern[(currentChannel-LOW_CHANNEL)*3+1] = 127;
	LEDpattern[(currentChannel-LOW_CHANNEL)*3+2] = 127;
...
// Returning "true" to the network stack says that this message should be acknowledged
// if the sender requested it, although broadcast messages generally should not require
// acknowledgement.
	return true;
}
